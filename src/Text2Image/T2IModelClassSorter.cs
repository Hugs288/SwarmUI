...
public class T2IModelClassSorter
{
    ...
    public static void Init()
    {
        ...
        Register(new() { ID = "anima/lora", CompatClass = CompatAnima, Name = "Anima LoRA", IsThisModelOfClass = (m, h) =>
        {
        ...
        if (arch is not null)
        {
            ...
            if (ModelClasses.TryGetValue(arch, out T2IModelClass clazz))
            {
                ...
                else
                {
                    ...
                    matchedClass = clazz with { CompatClass = clazz.CompatClass with { StandardWidth = width, StandardHeight = height }, IsThisModelOfClass = (m, h) => false };
                }
            }
            else
            {
