...
public partial class WorkflowGenerator
{
    ...
    public string CreateVAEDecode(JArray vae, JArray latent, string id = null, bool canAudioDecode = true)
    {
        ...
        if (UserInput.TryGet(T2IParamTypes.VAETileSize, out _) || UserInput.TryGet(T2IParamTypes.VAETemporalTileSize, out _))
        {
            return CreateNode("VAEDecodeTiled", new JObject()
            {
                ...
                ["temporal_size"] = UserInput.Get(T2IParamTypes.VAETemporalTileSize, CurrentCompatClass().StartsWith("wan-21") || CurrentCompatClass().StartsWith("wan-22") || CurrentCompatClass() == "hunyuan-video-1_5" ? 9999 : 32),
        ...
        else if ((CurrentCompatClass() is "hunyuan-video" or "hunyuan-video-1_5" or "kandinsky5-vidlite" or "kandinsky5-vidpro") && UserInput.Get(T2IParamTypes.ModelSpecificEnhancements, true))
        {
            return CreateNode("VAEDecodeTiled", new JObject()
            {
                ["vae"] = vae,
    ...
    public string CreateKSampler(JArray model, JArray pos, JArray neg, JArray latent, double cfg, int steps, int startStep, int endStep, long seed, bool returnWithLeftoverNoise, bool addNoise, double sigmin = -1, double sigmax = -1, string previews = null, string defsampler = null, string defscheduler = null, string id = null, bool rawSampler = false, bool doTiled = false, bool isFirstSampler = false, bool hadSpecialCond = false, string explicitSampler = null, string explicitScheduler = null, int sectionId = 0)
    {
        ...
        if (CurrentCompatClass() is "lightricks-ltx-video" or "lightricks-ltx-video-2")
        {
            if (!hadSpecialCond)
            {
        ...
        else if (CurrentCompatClass() == "flux-1-kontext" || CurrentCompatClass().StartsWith("omnigen-") || CurrentCompatClass().StartsWith("qwen-image") || CurrentCompatClass() == "flux-2")
        {
            ...
            if (img is not null)
            {
                ...
                else if (CurrentModelClass().ID.StartsWith("qwen-image-edit-plus"))
                {
                    neg = imgNeg;
                }
            }
    ...
    public JArray CreateConditioningDirect(string prompt, JArray clip, T2IModel model, bool isPositive, string id = null)
    {
        ...
        if (CurrentModelClass().ID is "hunyuan-video-skyreels" or "hunyuan-video-skyreels-i2v")
        {
            defaultGuidance = 1;
        }
        ...
        if (CurrentCompatClass().StartsWith("qwen-image-edit") && (isPositive || CurrentModelClass().ID.StartsWith("qwen-image-edit-plus")) && (qwenImage = GetPromptImage(true, true)) is not null)
        {
            if (wantsSwarmCustom)
            {
                ...
                if (CurrentModelClass().ID.StartsWith("qwen-image-edit-plus") && image2 is not null)
                {
                    string batched = CreateNode("ImageBatch", new JObject()
                    {
                        ["image1"] = qwenImage,
        ...
        else if (CurrentModelClass().ID is "hunyuan-video-i2v" or "hunyuan-video-i2v-v2" && prompt.StartsWith("<image:"))
        {
